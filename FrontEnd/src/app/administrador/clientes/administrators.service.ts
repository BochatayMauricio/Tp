...
export class AdministratorsService {
  ...
  postAdministrator(administratorEnter:administrator){
    ...
    return this.http.post('http://localhost:3000/api/Administrators',administratorEnter);
  }

  retraiveAdministrator(){
    this.http.get('http://localhost:3000/api/Administrators').subscribe(administrators=> {
  ...
  deleteAdministrator(administrator:administrator){
   ...
   return this.http.delete(`http://localhost:3000/api/Administrators/${dniAdministrator}`);
  }

  updateAdministrator(administratorModify:administrator){
    const dniAdministrator = administratorModify.dni;
    return this.http.put(`http://localhost:3000/api/Administrators/${dniAdministrator}`,administratorModify)
  }
}
